...
func set_and_animate(value:float, chunk:float, start_pos:Vector3, height:float = 2, spread:float = 1.5):
	...
	# "Play" the tween.
	tween.tween_property(labcon, "position", end_pos, tween_length).from(start_pos)
